...
- `opensAt` (default null; leaf metrics only, inside 0..`marketRangeMax`): where this metric's untraded books open, in place of the current reading. For a number with no running reading, e.g. a game's score, whose value between games is the last game's result. A traded open book of the same metric still wins, a proposal's branch still follows its baseline, and a book that already has a price is never moved. `null` goes back to the reading.
