...
- `timePreference.customHorizons` (optional, max 24 entries) adds explicit market dates beyond the curve: rolling offsets (`"+1h"`, `"+3m"`, `"+2w"`; re-resolved against now on every hourly refresh so there is always a market that far out) or one-shot absolute dates (`"2026-12-31"`, `"2026-12"`, `"2026-W50"`, `"2026"`, `"2026-12-31T14"` for the 14:00-15:00 UTC hour, `"2026-12-31T14:30"` for the single minute to 14:31). Custom horizons work even with `enabled: false` (pure manual horizons, no exponential curve). An intraday ladder is `["+1h", "+2h", ..., "+24h"]`. Minutes work too: `"+Nmin"` (1 to 1440) is a rolling minute horizon, which is what a floor deciding every minute prices on. A rolling entry only exists once the markets are refreshed, so a floor moving that fast forces it with `POST /api/predictions/markets/refresh {"force": true}` (a manager call, no cooldown) rather than waiting for the hourly cron. Removing an entry deactivates its market (sell-only); positions are kept and resolve normally. One more entry has no clock at all: `"until-settled"` is a date that settles only when the owner calls `POST /api/metrics/:id/settle {value, reason}`. Its book carries `targetDate: "until-settled"` and `resolvesOn: "9999-12-31T00:00:00Z"` (a far edge, not a real moment: tell it apart by the target date), never falls due on a reading or a deadline, never voids N/A, and once settled the next refresh opens a fresh one; at most one per metric, and settling the metric settles every open book on it, dated ones too. Use it for a question whose answer arrives at a moment only the owner can name, such as a game attempt's reached length.
...
- `timePreference.horizonTitles` (optional) is the words the floor reads for a date on its tab and in its question, in place of the clock's name, keyed the same way: `{"until-settled": "this attempt", "2026-12": "at the end of season 1"}`. Write it to follow the metric's name ("What will be Snake's reached length this attempt?"); at most 60 characters, blank is no title. The floor payload carries it per book as `dateTitle`. `timePreference` is written whole, so a PUT that omits `horizonCredits` or `horizonTitles` clears them.
...
# PATCH /api/agents/me/keys/:keyId { label?, scopes?, workspaceLocked? }, DELETE /api/agents/me/keys/:keyId (401 for that key from then on).
...
- **Never string-match an `error` message.** Wording is not stable. The errors
  ...
  `available`), `crosses_own_order` (409, RETIRED 2026-09-13, never
  returned: opposing limit orders are matched instead), `trade_too_small`, `market_not_found`, `market_resolved`,
